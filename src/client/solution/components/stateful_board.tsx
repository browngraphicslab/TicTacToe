...
import { observable, action, runInAction, reaction, IReactionDisposer, computed } from "mobx";
...
export default class StatefulBoard extends React.Component<StatefulBoardProps> {
    ...
    constructor(props: StatefulBoardProps) {
    ...
    private get slider() {
        const visibility = !this.elapsedMoves ? "visible" : "hidden";
        return (
            <input
                style={{ visibility }}
                type={"range"}
    ...
    render() {
        ...
        const height = !this.elapsedMoves ? "calc(100% - 40px)" : "100%";
        return (
            // literal JSX
